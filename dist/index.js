...
const processSelectedFile = async (file, folder) => {
    try {
...
const processSingleUploadFolder = async (resPath, name, parentId) => {
    if (fs.existsSync(resPath)) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (mimeType) {
                ...
                if (isFolder)
                    await processSingleUploadFolder(fullPath, fileName, folderId);
            }
        }
    }
    else {
...
const processFolderActions = async (folderName, repeatData) => {
...
const processTrashFile = async (fileId) => {
    try {
